...
- **PRESENT → arm the loop.** Requires VERIFIED enablement or activity — an installed app is NOT enough on its own (see below). One of: (a) the app is installed AND its config shows automatic PR review is actually ENABLED (the review toggle is on, not merely installed); OR (b) historical bot activity on recent PRs (a `chatgpt-codex-connector` review/comment on any recent PR of this repo) — proof it really posts here. Only then run the full procedure below.
- **ABSENT → skip the loop cleanly.** The inventory call SUCCEEDS authoritatively AND shows no such app installed at all. Do not arm a timer; the PR goes straight to the human. Note in the registry/hand-off: "no review bot wired — skipping bot-loop".
- **UNKNOWN → ask the human ONCE, then cache.** Anything short of verified enablement/activity but not a clean ABSENT — INCLUDING an app that is merely installed with no proof it reviews (install status alone can't tell enabled from disabled), a selected-repos install you can't see into, a non-owner `403`/`404` on the inventory call, a personal (non-org) repo, or simply no PR history to judge from. Ask the human once ("is a Codex/review bot actually reviewing PRs on this repo?"), and CACHE the answer for the rest of the session — do not re-ask per PR. Inventory (`gh api /orgs/<org>/installations` or `gh api /repos/<owner>/<repo>/installation` — needs a token with repo-owner + `read:org`) can CONFIRM installation, but installation alone lands here in UNKNOWN, never PRESENT.

**`installed` ≠ `review-enabled` — this is why installation alone => UNKNOWN, not PRESENT.** The app being installed and the app being configured to actually review PRs are SEPARATE facts (a separate toggle on the app). An installed-but-disabled bot never posts a comment; treating mere installation as PRESENT would arm a timer and loop FOREVER waiting on a review that never arrives. So an install with no enablement/activity proof is UNKNOWN — resolve it via the enabled-toggle check, historical-activity evidence, or the human's answer before ever arming the loop. Cache the two facts (installed vs review-enabled) separately.
...
2. **Sweep on fire.** FIRST confirm the PR is still yours: a human may have merged/closed/rebased it silently — `gh pr view <n> --json state,mergedAt,headRefOid`. If MERGED/closed, STOP and jump to "When a PR merges under you". Else read the bot's **reaction emoji first** (section below) — the quickest state signal, and it gates whether a nudge is allowed. Then fetch findings:
   ...
   - text clean-bill ("Codex Review: Didn't find any major issues" / "no major issues") is posted as an ISSUE COMMENT, not a formal review — a scan of only `--json reviews` will read "pending" forever and miss it; also read `gh api repos/<owner>/<repo>/issues/<n>/comments`. The body reaction above is the faster primary signal; this is its text confirmation.
...
5. **Fix as ONE cohesive pass, cross-review, THEN push — not per-bug.** Cluster fix-worthy findings by theme. DELEGATED path: send them to the SAME implementer conversation (reuse `agent`+`title`, `purpose:"implement"`) so it keeps its worktree/branch/PR, and tell it to fix + drive gates green ON THE BRANCH but NOT push yet. DIRECT-AUTHORING path (a doc/skill polly opened itself, no implementer sub-agent): polly revises the prose DIRECTLY on the branch. Either way, an already-open PR is still under the pre-push gate (see `review-before-pr` → "an ALREADY-OPEN PR is still under this gate"): apply review-before-pr to the fix diff — run the gates yourself (tsc/lint/build for code; the doc-lens gate for prose), then dispatch a DIFFERENT-vendor cross-review of the fix diff and loop blocking issues back to the fixer (the same implementer, or polly for a directly-authored artifact) — all BEFORE the push. Only when the fix diff is gate-green AND review-clean does polly record the fix commit in the worktree marker (`sys_os_shell("git -C .worktrees/<task_id> rev-parse HEAD > .worktrees/<task_id>/.polly/review-passed")`), which clears the `require_pr_review` gate so the fix is pushed to the open PR (by the implementer, or by polly for a directly-authored artifact) — the marker names this exact fix commit, so an unreviewed later commit is blocked again; THEN re-request bot review — a NEW top-level comment of exactly `@codex review` (`gh pr comment <n> --body "@codex review"`) — and re-arm the timer. Loop. The bot re-request is the LAST step, never a substitute for the internal different-vendor cross-review.

   **Known trap: do not narrow the fix-round review to "confirm the bot's findings are closed."** Dispatching the reviewer with only the bot's comments as scope is the single most common way this loop turns into multi-round whack-a-mole — each round only re-checks what the bot JUST found, so a fix's own siblings (a recurring defect class in the same predicate, a sibling render path, a stale doc claim) surface one bot round at a time instead of together. Every fix-round review — even one purely servicing bot comments — runs the SAME full battery as a first-pass review (`cross-review` → Procedure step 3, FOCUSED + all four WIDE axes, plus the mandatory battery-completeness checklist), with the bot's findings folded in as additional FOCUSED-pass items, not as the review's entire scope.
...
6. **Clean bill → stop.** Merge-clean requires ALL: (a) the latest review/reaction on the CURRENT head shows no new P1/P2 (a 👍 `+1` on the current head is the bot's explicit "no issues" verdict — see below), AND (b) ZERO UNRESOLVED bot threads (verify via GraphQL, not REST — a reply does NOT resolve, and `isOutdated` ≠ resolved), AND (c) the PR's REQUIRED CI checks are green (`gh pr view <n> --json statusCheckRollup`) — a 👍 clean bill does NOT gate on CI, so a genuine red required check (e.g. an e2e/DB-tier job that path-skips on other PRs but RUNS here because this diff touched its files) can sit under a clean verdict; tell a required red from a non-required infra flake before calling it ready. RESOLVE every handled thread (fixed / accepted-pushback / superseded) so the human sees a clean PR; leave open only a real finding you're still working. Mark ready in the registry, **disarm timers** (`sys_timer_cancel`), leave the merge to the human.
...
## Front-run the bot on rich surfaces
The bot does whole-state-space reasoning, so on a **combinatorially-rich surface** (serializer, form↔payload round-trip, state machine, multi-input derivation/filter) it keeps finding one uncovered state per round — turning what should be one fix into hours of code→review→PR→bot→patch. Don't let that class of finding FIRST appear in this slow post-PR loop. BEFORE the PR opens, run an internal ADVERSARIAL pass with the same engine (`codex`, a DIFFERENT vendor from the implementer) that enumerates the state space and attacks it — see `cross-review` → "Match review depth to the surface" and `review-before-pr` → "Rich surfaces". Front-running moves the bot's findings into the fast internal loop. If the bot still opens 3 rounds on the same rich surface here, that's the structural trigger — escalate to the design debate below rather than patching edge N+1.

## Notes
- This loop composes with `cross-review` (internal, pre-PR, different-vendor) and `fanout` (each parallel PR runs its own loop). Internal cross-review and the bot often overlap the same commit — merge their findings into ONE fix pass to avoid double churn.
