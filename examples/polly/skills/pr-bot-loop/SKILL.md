...
3. **Disposition EACH finding** (P1 > P2 > P3), exactly one of:
   ...
   - **accept-and-document** — real, but below the agreed blocking bar for this
     artifact's criticality (see `cross-review` → "Calibrate the blocking bar"):
     ...
     misled. A legitimate disposition, not a dodge — but bounded: it may NEVER
     absorb a finding that hits a non-demotable structural gate (parallel
     surface, input-domain, recurrence, obligated coupled artifact), and a P1
     accepted this way needs the HUMAN's explicit agreement, not polly's. When
     the bar is unclear, ask rather than assume.
