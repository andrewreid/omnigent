...
5. **Fix as ONE cohesive pass, cross-review, THEN push — not per-bug.** Cluster fix-worthy findings by theme; send to the SAME implementer conversation (reuse `agent`+`title`, `purpose:"implement"`) so it keeps its worktree/branch/PR, and tell it to fix + drive gates green ON THE BRANCH but NOT push yet. An already-open PR is still under the pre-push gate (see `review-before-pr` → "an ALREADY-OPEN PR is still under this gate"): apply review-before-pr to the fix diff — run the gates yourself (tsc/lint/build), then dispatch a DIFFERENT-vendor cross-review of the fix diff and loop blocking issues back to the same implementer — all BEFORE the push. Only when the fix diff is gate-green AND review-clean does the implementer push; THEN re-request bot review — a NEW top-level comment of exactly `@codex review` (`gh pr comment <n> --body "@codex review"`) — and re-arm the timer. Loop. The bot re-request is the LAST step, never a substitute for the internal different-vendor cross-review.
