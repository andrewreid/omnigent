...
| `OMNIGENT_DB_CONNECTION_SCOPE` | `1` | Per-request DB connection reuse. Set `0` to restore checkout-per-store-call behaviour (kill switch for suspected pool saturation). Apply with `docker compose up -d` — `docker compose restart` keeps the old container environment. |
