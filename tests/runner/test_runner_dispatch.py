...
class _FakeProcessManager:
    ...
    def __init__(self, harness_client: _FakeHarnessClient | None = None) -> None:
        ...
        # call, so tests can assert the actual spawn_env the process manager
        # would have launched the harness subprocess with — not just that a
    ...
    async def release(self, conversation_id: str) -> None:
        ...
        seeded instructions). Tracked so tests can assert it actually
...
async def test_instruction_delivery_warn_fires_once_for_undelivered_harness(
    ...
    identical across both turns, which under amendment A1 ("at most once per
    (conversation, effective instruction-delivery value)") is the arm that
    must stay at exactly one warning. Like its agent-switch sibling this is
    regression coverage rather than an A1 detector — the pre-A1 presence-only
    marker passes it too.
    """
...
async def test_instruction_delivery_warn_body_harness_override_wins_over_body_harness(
    ...
    path the harness_override-only test above exercises, which was already
    correct before this fix and would still pass if the new body-precedence
    code were reverted).
    """
...
async def test_opencode_native_wire_instructions_use_gated_composed_value(
    ...
    resolves — the same silently-dropped-caller-input defect this area
    exists to close, reintroduced by the swap.

    This test previously asserted the swap discarded the body value, on the
    assumption that anything already there was an internal fallback. A
    caller-supplied ``?stream=true`` body carries RAW per-request text, so
    that assumption was wrong and the assertion locked the bug in.
    """
    ...
    monkeypatch.setattr(
        runner_app_mod, "_auto_create_opencode_terminal", _fake_auto_create_opencode_terminal
    )
...
async def test_direct_stream_instruction_composition_uses_sub_agent_spec(
    ...
    harness (so this exercises the "harness already known" else-branch of
    ``_stream_message_to_harness``, not the ``_resolve_effective_turn`` path
    that a background-turn regression test would already cover).
    """
    import omnigent.runner.app as runner_app_mod
...
async def test_direct_stream_instruction_composition_ignores_stale_spec_cache_after_agent_switch(
    ...
    apply the same check — this test populates the cache via a real
...
async def test_direct_stream_ignores_spec_cache_with_no_recorded_provenance(
    ...
    unlike the sibling test above, this is not "stale from a KNOWN prior
    ...
    whatsoever, keeping this test meaningful.
    """
    import omnigent.runner.app as runner_app_mod
...
async def test_background_dispatch_ignores_spec_cache_with_no_recorded_provenance(
    ...
    """
    The BACKGROUND dispatch path (``_run_turn_bg_setup_and_stream``) has
    its own separate cache-provenance check from the direct-stream path
    (they don't share one resolver — see the docstring on
    ``_resolve_effective_turn``) and needs its own regression coverage.

    Genuine unknown provenance is populated via ``GET
    /v1/sessions/{id}/resources`` (same mechanism as the direct-stream
    sibling test above) — the direct-stream cold-boot path itself now
    records ``_session_agent_ids`` after resolving, so driving this via a
    direct-stream turn would just be an ordinary known A-then-B switch,
    not the unknown-provenance case this test targets. Turn 2, for a
    DIFFERENT agent, goes through the background dispatch path and must
    not trust that unknown-provenance cache entry.
    """
    import omnigent.runner.app as runner_app_mod
...
async def test_tool_schemas_cache_retries_after_unresolved_spec_turn() -> None:
    """
    Regression test for a stale-negative-cache gap: ``_session_tool_schemas``
    ...
    Proves the fix: an unresolved-spec turn leaves the cache UNSET, so the
    next turn (spec now resolvable) retries and populates real schemas.
    """
...
async def test_tool_schemas_mcp_only_partial_result_is_not_cached_as_final(
    ...
    only. Proves the fix: the failed turn caches nothing, and the next turn
...
async def test_agent_switch_closes_and_rebuilds_the_comment_relay(
    ...
    life of the session. Proves the fix: switching A -> B closes the A relay
...
async def test_list_session_resources_uses_recorded_agent_id_as_hint() -> None:
    """
    Regression test for a dropped-provenance-hint gap: ``list_session_resources``
    ...
    hint). Proves the fix: once a turn has dispatched agent A, a resources
...
async def test_initialize_session_arms_use_request_agent_id_as_hint(
    ...
    Pre-fix sensitivity: the snapshot GET 404s for the whole test, so a bare
    resolve raises ``OmnigentError`` and every arm degrades to no spec — the
    ``skills_filter`` asserted below can only arrive via the hint. The
    ...
    async def _capture_launch_native_terminal(
        ...
        **_kw: object,
        ...
        """pi / cursor / opencode / kimi arms pass a resolver callable."""
...
@pytest.mark.asyncio
async def test_background_dispatch_fails_turn_on_unresolvable_recovered_sub_agent_name() -> None:
    """
    Background dispatch (``_run_turn_bg_setup_and_stream``) must fail the
    turn — not silently fall back to the parent spec — when the recovered
    ``sub_agent_name`` (from a session snapshot, via ``_recover_sub_agent_name``)
    genuinely does not resolve in the spec tree.
    ...
    resolution that happens (asserted below) — a previous version of this
    test pre-populated the cache via session-create and then engineered a
    SECOND resolver call to return a worker-less tree, but the real
    background-dispatch code path only calls ``spec_resolver`` when the
    cache is empty, so that second call never actually happened; the
    reported "failure" was for an unrelated reason (a since-fixed bug where
    an already-resolved cached child was re-searched as if it were the
    root). This version resolves in one real call, matching production.
    """
    ...
    assert statuses[-1] == "failed", (
        "Background turn should fail loudly when the recorded sub_agent_name "
        "no longer resolves in the spec tree, not silently dispatch as the "
        "parent agent."
    )
    assert calls["n"] == 1, (
        ...
        f"would mean this test isn't reproducing the real single-resolution "
        f"code path."
    )
...
async def test_background_dispatch_succeeds_when_cache_already_holds_resolved_child() -> None:
    ...
    # adapter) — this test is about _find_spec_by_name's root self-check,
...
async def test_background_dispatch_evicts_rejected_cache_entry_not_just_agent_id(
    ...
    # sees the new agent). A snapshot mock hardcoded to always report agent
    # A regardless of what's actually dispatched would inject an unrelated,
    # unrealistic staleness source of its own — the point of this test is
    # the background path's own provenance-rejection handling, isolated
    # from that.
...
async def test_direct_stream_three_turn_switch_does_not_launder_stale_cache(
    ...
    own. This test drives a THIRD turn (still B) specifically because two
    ...
    # directly — so this lagging client exists to prove that decoupling,
    # not to be "worked around" by keeping it in sync.
...
def test_direct_and_background_switch_sites_share_one_invalidation_routine() -> None:
    ...
    ``/agent-cache/reset`` route — already covered). Three independently
    ...
    the others. This is a structural/source-level regression test (not a
    runtime behavioral one) because the caches in question — skills, tool
...
@pytest.mark.asyncio
async def test_direct_stream_composition_preserves_caller_instructions_on_unresolvable_sub_agent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    Direct ``?stream=true`` instruction composition (known-harness branch)
    must not fall back to the PARENT spec when the recorded
    ``sub_agent_name`` no longer resolves in the cached spec tree — it must
    treat this as indeterminate composition and leave caller-supplied
    ``instructions`` untouched, the same "positively resolved spec or None"
    contract every other consumer in this function follows.
    """
    import omnigent.runner.app as runner_app_mod
    ...
    root_spec_with_worker = AgentSpec(
        spec_version=1,
        name="root",
        instructions="Root instructions — must not be composed for this turn.",
        executor=ExecutorSpec(type="omnigent", config={"harness": "opencode-native"}),
        sub_agents=[
            AgentSpec(
                spec_version=1,
    ...
    assert (
        recorder.posted_bodies[-1]["instructions"] == "Caller-supplied instructions must survive."
    )


@pytest.mark.asyncio
async def test_no_harness_direct_stream_503s_on_unresolvable_sub_agent_name() -> None:
    """
    ``_resolve_effective_turn`` (the NO-harness direct-stream path) must
    raise, not silently dispatch as the parent, when the recorded
    ``sub_agent_name`` no longer resolves in the spec tree. The route
    catches ``RuntimeError`` from this resolver and returns 503
    ``spec_resolver_failed`` — this is the one path in this module where
    the miss escalates to an error response rather than degrading to
    "unknown", because here (unlike the composition-only Site C path) the
    harness itself is not yet independently known, so there is nothing
    safe to dispatch to.
    """
    root_spec_with_worker = AgentSpec(
        spec_version=1,
    ...
    assert response.status_code == 503, (
        f"Expected 503 spec_resolver_failed; got {response.status_code} "
...
@pytest.mark.asyncio
async def test_resolve_session_spec_entry_404s_on_unresolvable_sub_agent_name() -> None:
    """
    ``_resolve_session_spec_entry`` must raise, not silently return the
    PARENT spec, when the session snapshot's ``sub_agent_name`` no longer
    resolves in the freshly-fetched spec tree. Exercised end-to-end via
    ``GET /v1/sessions/{id}/resources``, one of the callers with no local
    ``except OmnigentError`` — the raise reaches the global
    ``OmnigentError`` handler and becomes a clean 404 JSON response.
    """
    ...
    assert response.status_code == 404, (
        f"Expected 404 from the global OmnigentError handler; got "
...
async def test_gated_harness_preserves_caller_instructions_when_resolver_returns_none(
    ...
    Sibling of the RuntimeError-based resolution-failure test above: only a
    *raised* exception set the preservation flag before this fix, so a
    resolver that legitimately returns ``None`` fell straight through to the
...
async def test_gated_harness_preserves_caller_instructions_with_no_agent_id(
    ...
    Before this fix, composition was only marked "unresolved" when BOTH an
    agent id AND a configured resolver were present (the only branch that
    could set the old resolution-failed flag) — a missing agent id alone
    left the flag ``False`` and fell straight through to the gated
    wire-swap, silently stripping instructions the caller already supplied.
...
async def test_instruction_delivery_warn_does_not_double_fire_after_agent_switch(
    ...
    Amendment A1 scopes the dedup to
    ``(conversation, effective instruction-delivery value)``. BOTH agents here
    ...
    This test is regression coverage, NOT a detector of the A1 change: the
    pre-A1 presence-only ``set[str]`` marker also emits exactly one warning for
    a same-delivery switch, so it passes either way. The two tests that ARE
    sensitive to A1 are
...
async def test_hermes_wire_instructions_use_gated_composed_value() -> None:
    ...
    that already includes this turn's per-request text. Like its sibling,
    this test used to assert the caller's value was discarded and so locked
    in the drop.
    """
...
async def test_direct_stream_genuine_harness_swap_rebuilds_spawn_env_for_process(
    ...
    dict ``process_manager.get_client`` was called with, closing the gap
    where prior coverage only checked warning text while the fake process
    manager discarded env entirely.
    """
    monkeypatch.setenv("OMNIGENT_CONFIG_HOME", str(tmp_path))
    monkeypatch.setenv("OMNIGENT_DISABLE_KEYRING", "1")
...
@pytest.mark.asyncio
async def test_create_session_unknown_sub_agent_name_returns_404() -> None:
    """
    ``POST /v1/sessions`` with a ``sub_agent_name`` absent from the spec
    tree must 404, not silently create the session bound to the PARENT
    spec (wrong harness/instructions masquerading as the requested
    sub-agent).
    """
    sub_spec = AgentSpec(
        spec_version=1,
    ...
    assert response.status_code == 404, (
        f"Expected 404 sub_agent_not_found; got {response.status_code} "
        f"with body {response.text!r}."
    )
    assert response.json()["error"] == "sub_agent_not_found"


@pytest.mark.asyncio
async def test_create_session_sub_agent_name_matching_root_name_returns_404() -> None:
    """
    ``POST /v1/sessions`` with a ``sub_agent_name`` that happens to equal
    the ROOT spec's own name (but the root has no matching CHILD) must
    still 404 — a coincidental name match at the root is not a resolved
    sub-agent.

    Regression: ``_find_spec_by_name`` briefly self-matched its own
    ``spec`` argument against ``name`` before searching children, added to
    fix an unrelated idempotency bug (re-resolving an ALREADY-RESOLVED
    cached child spec against itself). That self-match had no way to tell
    "fresh root that happens to share a name" apart from "already-resolved
    child", so a top-level coordinator named e.g. "worker" with no
    "worker" sub-agent would let ``POST /v1/sessions`` with
    ``sub_agent_name="worker"`` return 201 bound to the PARENT's spec —
    silently bypassing this exact route's own ``sub_agent_not_found``
    guard just above.
    """
    root_spec = AgentSpec(
        spec_version=1,
    ...
    assert response.status_code == 404, (
        f"A root whose own name coincidentally matches sub_agent_name, with "
        f"no genuine child by that name, must 404 — not silently accept the "
        f"root as the resolved sub-agent. Got {response.status_code} with "
...
async def test_reset_state_evicts_the_session_init_envelope(
    ...
    the TTL. Proves the fix: after ``/reset-state`` a turn dispatched as B
    ...
    monkeypatch.setattr(runner_app_mod, "_auto_create_claude_terminal", _noop_async)
    monkeypatch.setattr(
        runner_app_mod, "_claude_native_terminal_arrives_via_transfer", _no_transfer
    )
    monkeypatch.setattr(
...
async def test_session_scoped_reads_resolve_the_recorded_agent_not_the_snapshot(
    ...
    Pre-fix sensitivity: the snapshot here reports B for the whole test while
    the recorded marker says A, so dropping the hint from the handler under
    test sends B to the resolver and fails the assertion.
    ...
    class _StaleBSnapshotClient(NullServerClient):
        """Snapshot lags on agent B for the life of the test."""
...
async def test_instruction_delivery_warn_refires_when_the_switch_changes_delivery(
    ...
    Sibling of ``test_instruction_delivery_warn_does_not_double_fire_after_agent_switch``,
    which pins the other half: a switch where the delivery gap is identical
    stays at one warning per conversation. Both are the same rule — dedup on
    ...
    Pre-fix sensitivity: verified against the previous ``set[str]`` marker,
    where B's turn found ``conv_id`` already present and emitted nothing —
    one warning total instead of two.
    ...
    assert len(undetermined) == 1, (
        ...
        f"marker suppressed a warning about a gap it never covered."
    )
...
async def test_invalidate_midflight_drops_the_stale_fill(writer: str) -> None:
    ...
    Pre-fix sensitivity: verified by making ``_session_cache_put`` skip its
    generation check and publish unconditionally — both parametrizations then
    report exactly one resolver call and fail.
...
async def test_instruction_delivery_warn_does_not_refire_for_a_recurring_value(
    ...
    Amendment A1 is a LIFETIME rule: at most once per (conversation, effective
    instruction-delivery value) for as long as the conversation lives. A marker
    ...
    Pre-fix sensitivity: verified by reverting the marker to a single
    last-warned pair (``_instruction_delivery_warned[conv] = _warned_for`` plus
    an ``== _warned_for`` gate) — the third turn then emits a second
    not-delivered warning for A and this test fails at 2 != 1.
...
async def test_skills_fill_parked_past_invalidation_does_not_publish(
    ...
    Isolation: removing ONLY the skills guard fails this test. Removing only
    the spec guard does not — the skills cache is still evicted by the reset,
    so discovery re-runs and the assertion holds.
...
# ── Cross-path resolution contract (plan amendment A2-R1) ────────────────
#
...
#
# They are NOT unified, and A2 withdrew the requirement that they should be: a
# read-only measurement found unification needs nine independently varying
# behaviour policies, and that no zero-behaviour-selection refactor can preserve
# current semantics, because the paths have observably OPPOSITE outcomes for a
# missing child and for a raising resolver. That divergence is correct — the
# three carry different transport contracts (a synchronous error response, a
# failure arriving asynchronously against an already-sent 202, and deliberate
# graceful degradation).
#
# The real defect was that the divergence was undocumented and unpinned, so any
# path could drift silently. The table below pins it. It deliberately does NOT
# assert sameness where the semantics differ: asserting sameness would be false
# and would force a bad unification later. Changing any single path's behaviour
# should surface here as a visible contract change in review.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # ── Missing child: INTENTIONAL DIVERGENCE. Do not "fix" this into
        # agreement — the three transports cannot report the same way.
        pytest.param(
            "child_missing",
            "no_harness",
            {"status": 503, "error": "spec_resolver_failed"},
            id="child_missing-no_harness-503",
        ),
        pytest.param(
            "child_missing",
            "known_harness",
        ...
        pytest.param(
            ...
            #
            # ``resolver_calls == 2`` is pinned deliberately. A None result is
            # NOT cached — caching it would be exactly the negative-caching
            # defect closed elsewhere on this branch — so each consumer
            # re-asks: once in background setup, once again for composition.
        ...
        # Pinned SEPARATELY, per A2, and it genuinely differs: this path does
...
async def test_cross_path_resolution_contract(
    ...
    Plan amendment A2-R1. See the block comment above for why the three paths
    are deliberately NOT unified and why this table asserts divergence rather
    than sameness.

    Reading the table: ``child_present`` is the only scenario where all three
    agree. Every other row records a transport-driven difference that is
    intended — a synchronous 503, graceful degradation preserving the caller's
    own instructions, and an asynchronous terminal ``failed`` against a 202
    that has already gone out. If you change one path and a row here starts
    failing, that is the point: decide whether the contract moved, do not
    quietly re-align the table.
    ...
    for key, want in expected.items():
        assert result.get(key) == want, (
            ...
            f"update the matrix deliberately — do not re-align it to make the "
            f"three paths agree, they are meant to differ."
        )
...
async def test_background_turn_composes_agent_and_per_request_instructions() -> None:
    ...
    Both other per-request regression tests drive direct ``?stream=true``, and
    the cross-path background adapter sends no ``instructions`` field at all,
    so nothing exercised the background path's own threading of the raw
    ...
    caller's instructions — the defect this work exists to close — and would
    have gone unnoticed.
    """
