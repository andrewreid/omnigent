...
async def test_instruction_delivery_warn_body_harness_override_wins_over_body_harness(
    ...
    path the harness_override-only test above exercises, which does not
...
async def test_background_dispatch_fails_turn_on_unresolvable_recovered_sub_agent_name() -> None:
    ...
    resolution that happens (asserted below). Background dispatch calls
...
async def test_gated_harness_preserves_caller_instructions_when_resolver_returns_none(
    ...
    Sibling of the RuntimeError-based resolution-failure test above. If only
...
async def test_instruction_delivery_warn_does_not_double_fire_after_agent_switch(
    ...
    This test is regression coverage, NOT a detector of the pair-keyed dedup:
...
async def test_cursor_model_names_fill_parked_past_invalidation_does_not_publish(
    ...
    part of what this pins. Reset while parked, release, then drive a model
    ...
    async def _no_op_auto_create(*args: object, **kwargs: object) -> None:
        ...
        # CLI this test does not provide.
        del args, kwargs
...
async def test_cross_path_resolution_contract(
    ...
    See the block comment above for why the three paths are deliberately NOT
    unified and why this table asserts divergence rather than sameness.
