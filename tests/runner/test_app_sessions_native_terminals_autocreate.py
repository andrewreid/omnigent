...
async def test_auto_create_claude_terminal_passes_raw_instructions(
    ...
    Regression guard for the managed-host wiring in
    ``runner/native/orchestration.py`` (``augment_claude_args(...,
    append_system_prompt=_native_startup_raw_instructions_from_spec(agent_spec))``):
    a revert of that call-site wiring would leave ``AgentSpec.instructions``
    silently unreachable by claude-native, and this is the only test that
    exercises the real ``_auto_create_claude_terminal`` → ``augment_claude_args``
    integration rather than the pure helper functions in isolation.
    """
    monkeypatch.setattr(claude_native_bridge, "_TRUSTED_PARENT", tmp_path)
    monkeypatch.setattr(claude_native_bridge, "_BRIDGE_ROOT", tmp_path / "root")
    monkeypatch.setenv("RUNNER_SERVER_URL", "http://127.0.0.1:8000")
...
async def test_auto_create_claude_terminal_does_not_cache_transient_resolver_failure() -> None:
    """
    Regression test for a stale-negative-cache gap: a ``resolve_launch_config``
    ...
    lifetime. Proves the fix: a resolver exception leaves the cache UNSET,
