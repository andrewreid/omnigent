...
async def test_failed_snapshot_does_not_pin_workspace_cache_to_none(
    ...
    """
    Regression test for a never-retried-because-already-present gap:
    ...
    key was already present. Proves the fix: a failed snapshot leaves the
    workspace cache UNSET, so the next call re-resolves and picks up the
    session's real, server-reported workspace.
    ...
    # from the (separately covered) spec-cache retry behavior.
    app = create_runner_app(
