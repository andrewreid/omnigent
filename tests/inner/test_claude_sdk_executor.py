...
def test_client_does_not_refresh_late_framework_instructions() -> None:
    ...
    # What matters is the second-turn options the SDK actually sees. The
    # client count is deliberately unconstrained: reusing one client and
    # rebuilding it on a changed composed prompt both satisfy the contract,
    # so pinning an exact count would assert an implementation choice this
    # test does not govern.
