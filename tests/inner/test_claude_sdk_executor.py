...
def test_client_does_not_refresh_late_framework_instructions() -> None:
    ...
    # This exercises the second-turn options actually seen by the SDK — the
    # only assertion the merge gate cares about. Deliberately NOT asserting
    # exactly one client: today's bug constructs exactly one (reused,
    # options frozen at turn 1); the eventual real fix may rebuild the
    # client on a changed composed prompt, constructing a second one with
    # fresh options. Pinning "== 1" here would keep this test XFAIL forever
    # even after the real fix lands (the count assertion would itself start
    # failing), defeating the merge gate's purpose.
