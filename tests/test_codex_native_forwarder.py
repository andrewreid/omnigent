...
def test_refresh_developer_instructions_from_config_present_to_absent_transition(
    ...
    Regression guard for the recurring class this closure targets: the
    forwarder must not just handle a single read correctly, but must
...
def test_read_developer_instructions_state_unreadable_missing_file(tmp_path: Path) -> None:
    """A missing config.toml reads UNREADABLE, not ABSENT."""
    from omnigent.codex_native_bridge import (
        DeveloperInstructionsReadState,
        read_codex_config_developer_instructions_state_from_home,
    )
...
def test_read_developer_instructions_collapsed_wrapper_matches_tri_state_value(
    ...
    must refuse/503 on UNREADABLE rather than guess). This test covers the
...
def test_default_collaboration_mode_reuses_current_developer_instructions() -> None:
    ...
    Regression for the self-inflicted overwrite bug: before this fix,
    ``_default_collaboration_mode`` always sent ``developer_instructions:
    null``, which Codex's app-server applies literally — clearing whatever
    ``build_codex_native_server`` had persisted the instant the user (or the
    plan-implementation flow) triggered a Default-mode turn.
    """
    state = fwd._CodexForwarderState()
    state.model = "gpt-5.4"
