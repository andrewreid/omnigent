...
async def test_replaced_connection_cleanup_does_not_offline_new_connection(
    ...
    registry = HostRegistry()
    store = HostStore(db_uri)
    disconnects: list[tuple[str, str | None]] = []

    async def _on_disconnect(host_id: str, user_id: str | None) -> None:
        disconnects.append((host_id, user_id))
    ...
    app.include_router(
        create_host_tunnel_router(
            registry,
            store,
            on_host_disconnect=_on_disconnect,
        ),
        prefix="/v1",
    )
    ...
    await asyncio.wait_for(_wait_offline(store, _HOST_ID), timeout=2.0)
    assert registry.get(_HOST_ID) is None
    assert disconnects == [(_HOST_ID, "local")]
