...
def test_subagent_parent_runner_id_if_diverged_detects_stale_binding() -> None:
    """
    The proactive check surfaces the parent's live id when it has diverged.

    A runner-replaced-mid-session scenario where only the parent was
    PATCH-rebound: the child still carries the old id, so the caller must
    route the very first forward through the parent's new one rather than
    spending a doomed attempt against the child's stale binding.
    """
    child = _conv("conv_child", runner_id="runner_old", parent_id="conv_parent")
    store = _FakeStore(_conv("conv_parent", runner_id="runner_new"))

    diverged = asyncio.run(_subagent_parent_runner_id_if_diverged(child, store))  # type: ignore[arg-type]

    assert diverged == "runner_new"


def test_subagent_parent_runner_id_if_diverged_none_when_matching() -> None:
    """No divergence (child and parent share the live id) reports None."""
    child = _conv("conv_child", runner_id="runner_same", parent_id="conv_parent")
    store = _FakeStore(_conv("conv_parent", runner_id="runner_same"))
