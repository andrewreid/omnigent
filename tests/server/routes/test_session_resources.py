...
async def test_native_subagent_boot_failure_recovers_via_diverged_parent_runner() -> None:
    ...
    id and only the parent was rebound) must route straight through the
    parent's live runner, run the parent's session-init handshake, and heal
    the child's binding — not just log the boot failure and stop.
    """
    ...
    # The parent's session-init handshake ran, then the failed edge landed
    # on the child's events URL — both through the resolved PARENT client,
    # never the stale one (there is only one fake client here; a direct
    # forward attempt against the un-healed child id would 404/produce a
    # default canned response indistinguishable from this, so the call
    # ORDER and the rebind below are what actually prove recovery ran
    # rather than a lucky direct hit).
    assert fake_runner.calls == [
        ("POST", "/v1/sessions"),
        ("POST", "/v1/sessions/child_boot_fail/events"),
    ]
    forward_url, forward_body = fake_runner.post_json_calls[-1]
...
async def test_native_subagent_boot_failure_recovers_via_missing_parent_inbox_503() -> None:
    ...
    ``create_session`` for). The shared helper must retry through
    :func:`_recover_subagent_status_forward_via_parent` rather than
    surfacing the 503 straight through.
    """
    from omnigent.server.routes.sessions import _forward_native_subagent_terminal_failure
