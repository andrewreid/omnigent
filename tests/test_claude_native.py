...
def test_run_with_local_server_threads_raw_instructions_to_prepare_terminal_fresh(
    ...
    faked. Existing coverage either fakes ``_prepare_claude_terminal``
    entirely (so a revert of the outer wiring stays green) or calls
    ``_prepare_claude_terminal`` directly with a hand-supplied
    ``append_system_prompt`` (so the outer→inner threading itself is never
    exercised). Only ``_launch_claude_terminal`` — one level further in — is
    faked here, to observe what the real ``_prepare_claude_terminal`` call
    actually forwards.
    """
    workspace = tmp_path / "workspace"
    workspace.mkdir()
