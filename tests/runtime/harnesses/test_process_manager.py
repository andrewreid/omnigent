...
async def test_get_client_seeds_model_and_reuses_without_respawn(
    ...
    This is the Part 1 no-respawn guarantee for issue #1907: when
    ``create_session`` seeds the initial spawn with the persisted
    ``/model`` override, the first turn requesting that same model must
    NOT tear the subprocess down and respawn it.
    """
