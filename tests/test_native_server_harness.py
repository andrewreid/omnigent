...
async def test_run_turn_and_enqueue_share_lock_no_lost_update() -> None:
    ...
    *before* it sends — the exact window ``_inject_lock`` must hold across.
    ...
    already held the "current" system prompt — an order inversion that
    asserting only "system_prompt is well-formed" (the previous version of
    this test) could never catch, since plain attribute writes can't tear
    under asyncio's cooperative scheduling regardless of the lock.
    """
    import asyncio
