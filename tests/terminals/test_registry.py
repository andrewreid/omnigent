...
async def test_close_expected_leaves_a_successor_on_the_same_key_running(
    ...
    reg._by_conversation["conv_fence"] = {("claude", "main"): successor}
    reg._instance_locks[("conv_fence", "claude", "main")] = threading.Lock()

    closed = await reg.close("conv_fence", "claude", "main", expected=superseded)
    ...
    assert reg.get("conv_fence", "claude", "main") is successor, (
...
async def test_close_expected_closes_its_own_instance(tmp_path: Path) -> None:
    ...
    reg._by_conversation["conv_fence_own"] = {("claude", "main"): mine}
    reg._instance_locks[("conv_fence_own", "claude", "main")] = threading.Lock()

    closed = await reg.close("conv_fence_own", "claude", "main", expected=mine)

    assert closed is True
    assert mine.closed is True
    assert reg.get("conv_fence_own", "claude", "main") is None
