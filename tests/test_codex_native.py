...
def test_clear_context_plan_implementation_refuses_before_creating_thread_when_unconfirmed(
    ...
    accepted but do nothing. This test asserts NO ``thread/start`` (or any
...
async def test_prepare_codex_terminal_fresh_session_passes_developer_instructions(
    ...
    Regression guard for the direct-wrapper call site in
    ``_prepare_codex_terminal`` (``codex_native.py``): a revert of that
    parameter threading would leave the CLI-launched path silently
    discarding the value even though the managed-host path still received
    it — this is the only test exercising that specific call site.
    """
