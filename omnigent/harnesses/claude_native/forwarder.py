...
async def forward_claude_transcript_to_session(
    *,
    ...
    async with (
        ...
        while True:
            try:
                # Reaped before the idle gate below: a finished child-history
                # worker is a task event, not a file change, so the gate cannot
                # see it — skipping first would strand the result and stall
                # child forwarding for as long as the inputs stay quiet.
                ...
                # Re-baseline the gate: the interrupted step must be retried,
                # so the next tick has to run its body even if nothing changed.
                fingerprint = None
                _logger.warning(
