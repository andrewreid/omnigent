...
class HermesExecutor(Executor):
    ...
    async def run_turn(
        ...
        # re-prefixing once a session id is captured below. Caveat carried
        # forward, not resolved by this fix: the prefixed text travels via
        # the CLI's -q argv, visible in process listings on the host — the
        # same class of exposure any --append-system-prompt-style flag
        # already has.
