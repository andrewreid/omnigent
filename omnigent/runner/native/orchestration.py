...
def _native_startup_raw_instructions_from_spec(
    ...
    text only — never the fully framework-composed per-turn string, which
    does not exist yet at terminal-launch time (late-bound framework text
    like ``SHARED_SESSION_AUTHORSHIP_INSTRUCTION`` is selected dynamically
    per-conversation immediately before each turn).
